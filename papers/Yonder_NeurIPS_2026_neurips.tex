...
% NeurIPS 2026 Evaluations & Datasets Track template
\PassOptionsToPackage{numbers}{natbib}
\usepackage[eandd]{neurips_2026}

% For double-blind submission (default), keep as above.
% For single-blind submission (allowed for ED dataset papers), use:
% \usepackage[eandd, nonanonymous]{neurips_2026}
% For camera-ready, add 'final':
...
\begin{tabular}{llrrl}
\toprule
Source & Upstream license & Scenes & Waypoints & Has Semantics \\
\midrule
...
A full Datasheet for Yonder, structured per~\cite{gebru2021datasheets}, is provided in Appendix~\ref{sec:datasheet}.
...
\begin{table}[h]
\centering
\caption{Offline detection performance on Yonder test set. N/M = not measurable due to confidence calibration collapse.}
\label{tab:offline}
\begin{tabular}{lccc}
\toprule
Model & mAP@0.5 & mAP@0.75 & Blank max score \\
\midrule
OWL-ViT v2 zero-shot & 4.8\% & 2.7\% & 0.42 \\
OWL-ViT v2 fine-tuned (A1) & N/M & N/M & \textbf{0.97} \\
Grounding DINO zero-shot & $\sim$5\% & --- & 0.39 \\
Grounding DINO fine-tuned R6 (A2) & \textbf{46.7\%} & \textbf{26.9\%} & 0.115 \\
...
\begin{table}[h]
\centering
\caption{Closed-loop navigation success rate (5 seeds, 465 trials per condition).}
...
Zero-shot Grounding DINO & \textbf{32.9\%} & \textbf{[28.8, 37.2]} & baseline \\
Fine-tuned OWL-ViT v2 (A1) & 11.6\% & [8.8, 14.4] & $<$0.001 (worse) \\
Fine-tuned GDINO R6 (A2) & 23.5\% & [19.0, 28.4] & 0.003 (worse) \\
Fine-tuned GDINO R6+clamp (A4) & 25.5\% & [20.7, 30.5] & 0.012 (worse) \\
Fine-tuned GDINO R7 (A3) & 25.5\% & [20.7, 30.5] & 0.012 (worse) \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{ack}
The Yonder dataset is publicly available at \url{https://huggingface.co/datasets/coybothf/yonder}. All fine-tuned model checkpoints, closed-loop benchmark code, and complete trial logs are available alongside the dataset. The released data is licensed CC-BY-NC-4.0 (inheriting HSSD's NonCommercial restriction); the code release is Apache-2.0. Compute for dataset generation was provided by the authors' organization (withheld for double-blind review); rented from Vast.ai at approximately \$60 total cost for the released subset.
\end{ack}

% References use unnumbered first-level heading per NeurIPS format.
% (thebibliography auto-emits the heading; do not also \section*.)
...
S. Liu, Z. Zeng, T. Ren, F. Li, H. Zhang, J. Yang, C. Li, J. Yang, H. Su, J. Zhu, and L. Zhang.
...
H. Huang, F. Zhao, S. Anand, Y. Wang, and L. Zhang.
...
P. Anderson, Q. Wu, D. Teney, J. Bruce, M. Johnson, N. S{\"u}nderhauf, I. Reid, S. Gould, and A. van~den Hengel.
...
\textit{Communications of the ACM}, 64(12):86--92, 2021.
...
\section{Datasheet for Yonder}
\label{sec:datasheet}

Following~\cite{gebru2021datasheets}:

\textbf{Motivation.} Created to enable training of drone-perspective perception models for autonomous indoor navigation. Creating organization and funder withheld for double-blind review.

\textbf{Composition.} 4,650,324 frames across 167 indoor 3D scenes (all from HSSD), organized as 387,527 waypoint NPZ files (52 sensor arrays per file plus 12 per-pixel semantic arrays per file, with semantic instance labels for all 167 scenes).

\textbf{Collection process.} Generated by flying a simulated Holybro x500v2 quadcopter through 3D scenes in Habitat-Sim. Waypoint sampling uses the navmesh with adaptive densification. Sensor capture is deterministic given a scene and seed.

\textbf{Preprocessing.} Raw renders compressed to NPZ with gzip level-1. Semantic channels re-rendered separately and projected to COCO bounding boxes by extracting bounding boxes around each unique semantic instance ID with area $>$ 100 pixels.

\textbf{Uses.} Recommended for perception model training with closed-loop validation in deployment domain. Not recommended for end-to-end navigation policy training or for reporting fine-tuning improvements based solely on offline metrics.

\textbf{Distribution.} Hosted on the HuggingFace Hub at \url{https://huggingface.co/datasets/coybothf/yonder}. Croissant metadata with Responsible AI fields published alongside the dataset card. License: \textbf{CC-BY-NC-4.0} (data, inheriting HSSD's NonCommercial restriction; HSSD attribution preserved per source license) and Apache-2.0 (code). Replica and HM3D-derived scenes were excluded because their upstream licenses do not permit open redistribution of derivative renders; ReplicaCAD scenes were excluded because they lack semantic annotations and were not used in any reported experiment.

\textbf{Maintenance.} Maintained by the corresponding author. Updates and corrections will be posted to the project website.

...
**Runner**: `eco/e2e/run_live_mission.sh` → `CloudBrainLiveMissionTests` (presidio-sdk,
...
- This proves the **cloud tier**. The **on-device tier** (OnDeviceBrain) still lacks a
  ...
  in**: `xcodebuild test -scheme presidio-sdk-Package -destination 'platform=iOS,id=<udid>'

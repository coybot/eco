...
## Livery. Split so the two surfaces read differently in motion: the fuselage is
## the roll axis and the wings are what actually show the bank, so giving them
## contrasting hues makes a turn legible from the chase camera at distance.
const BODY_COLOR := Color(0.42, 0.10, 0.78)    # purple
const WING_COLOR := Color(1.00, 0.38, 0.70)    # pink
...
func _ready() -> void:
	...
	], [[0,2,4],[5,3,1],[0,1,5,4],[4,5,3,2],[0,2,3,1]], WING_COLOR)
	...
	], [[0,2,3,1],[4,5,7,6],[0,1,5,4],[2,6,7,3],[0,4,6,2],[1,3,7,5]], WING_COLOR)
...
func _build_part(part_name: String, points: Array, faces: Array, color: Color) -> MeshInstance3D:
	...
	mat.metallic = 0.2
	mat.roughness = 0.5
	# A little emission so the airframe keeps its colour in shadow and against a
	# bright sky. The aircraft is what the eye is meant to follow and a flat grey
	# model over green ground at sixty metres reads as a smudge.

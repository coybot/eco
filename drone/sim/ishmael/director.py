...
class DirectorConfig:
    # "presidio_sim" = the project's own Godot engine (eco/drone/sim/godot_launch_fleet.py;
    #   PresidioEcoSim project). Godot itself is cross-platform (Linux/macOS/Windows) — this
    ...
    engine: str = os.environ.get("ISHMAEL_SIM_ENGINE", "presidio_sim")
    ...
    def resolved_python(self) -> str:
        ...
        return sys.executable  # presidio_sim: any regular Python env (see godot_launch_fleet.py)
...
def _launch_fleet(spec: TestSpec, cfg: DirectorConfig, result: TestResult):
    ...
    if spec.photoreal:
        if cfg.engine == "isaac":
            ...
            result.log("photoreal requested but ignored: presidio_sim (Godot) has no "

...
```bash
# (run with AWS_PROFILE=presidio on a machine with the AWS CLI)
ID=sim-quadcopter-001                 # your new drone id
AWS_PROFILE=presidio aws iot create-thing --thing-name $ID
ARN=$(AWS_PROFILE=presidio aws iot create-keys-and-certificate --set-as-active \
  --certificate-pem-outfile device.pem --private-key-outfile private.key \
  --query certificateArn --output text)
AWS_PROFILE=presidio aws iot attach-policy --policy-name drone-policy-dev --target "$ARN"
AWS_PROFILE=presidio aws iot attach-thing-principal --thing-name $ID --principal "$ARN"
...
AWS_PROFILE=presidio aws dynamodb put-item --table-name drone-registry-dev --item '{
...
AWS_PROFILE=presidio python3 fleet.py --fleet quad:10,rover:10 --user-sub <COGNITO_SUB>

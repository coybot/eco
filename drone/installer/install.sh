...
#
# Drone API Installer
# One-liner installation: sudo /bin/bash -c "$(curl -fsSL https://presidio-drone-installer.s3.amazonaws.com/install.sh)"
#
set -e
...
if [ "$EUID" -ne 0 ]; then
    ...
    echo ""
    echo "   sudo /bin/bash -c \"\$(curl -fsSL https://presidio-drone-installer.s3.amazonaws.com/install.sh)\""
    echo ""
    exit 1
fi
...
# Configuration
S3_BASE="https://presidio-drone-installer.s3.amazonaws.com"

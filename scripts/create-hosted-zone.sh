#!/usr/bin/env bash
# Create the Route53 hosted zone for coybotautonomy.com and print the
# nameservers to paste into GoDaddy. Run this once, before any `sst deploy`
# for the coybot-website app — SST's ACM cert validation needs the zone
# (and, once you've updated GoDaddy, the delegation) to already exist.
#
# Usage: ./create-hosted-zone.sh
set -euo pipefail

DOMAIN="coybotautonomy.com"
PROFILE="${AWS_PROFILE:-coybot}"
...
if [ -n "$existing" ]; then
  ...
  caller_ref="coybot-autonomy-$(date +%s)"
...
echo
echo "Next steps:"
echo "  1. Log into GoDaddy, open coybotautonomy.com DNS settings, switch to these 4 custom nameservers."
echo "  2. In the same sitting, recreate your GoDaddy email-forwarding MX records inside this Route53"
echo "     zone (aws route53 change-resource-record-sets) so hello@coybotautonomy.com keeps working —"

...
ROS 2 package (`drone/ros2_ws/src/coybot_drone/`), and you can fly the
...
- **ArduPilot SITL** with the
  ...
  or `mavros` bridge.
- This repo's `drone/ros2_ws/src/coybot_drone/` ROS 2 package.
...
Sim's physics. The `coybot_drone` ROS 2 node bridges Nav2 commands into
...
`coybot_drone` subscribes to Nav2's `/cmd_vel` and forwards it as MAVLink

...
|---|---|---|
| `set_config_path` | `(path)` | Override the directory where `config.yaml`, `certs/`, and logs live — for running as an installed package with config elsewhere. Equivalent to the `PRESIDIO_SDK_CONFIG_DIR` env var. |
...
from `config.yaml` (or `PRESIDIO_SDK_SERIAL_PORT`/`PRESIDIO_SDK_BAUD_RATE` env
...
to check (USB/serial path, or — for SITL — to set `PRESIDIO_SDK_SERIAL_PORT`).

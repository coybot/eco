...
(`control/{handler,conversations,drones,groups,rover,llm}.py` and
`control/backends/aws.py`) and the drone/app sides that talk to it. It does
**not** ship a deployable CloudFormation/SAM template — that's part of
Presidio's own private deployment, not published here. To run the AWS
backend yourself, you'll deploy your own infrastructure:

- **Lambda functions** wrapping `control/`'s handler functions — see
...
- **DynamoDB tables** — 5 tables, keyed exactly as
  `control/backends/local.py`'s `DEFAULT_TABLE_SCHEMAS` documents (that's
...
- **Auth**: `control/`'s handlers expect

...
1. **Boot with no WiFi configured** → the drone starts a WPA2/WPA3 hotspot
   (`Coybot-<model>-<last4>` by default) for `provisioning_timeout` seconds
...
```json
{"drone_id": "drone-abc123", "mac_address": "aa:bb:cc:dd:ee:ff", "hotspot_name": "Coybot-quad-ee01"}
```
...
- `user_id`: optional — identifies who's claiming the drone (AWS mode; used
  for ownership checks in `aws/src/drones.py`).

...
The drone daemon, the iOS/Android app, and the `control/` package all read
...
`control/clients.py` is a backend-selection seam. `control/handler.py`,
...
- **`control/backends/aws.py`** — real, lazily-constructed boto3 clients:
  DynamoDB, IoT Core (`iot-data`), S3. This is the default; nothing has to be
  configured for it.
- **`control/backends/local.py`** — `LocalDynamo` (SQLite), `LocalIoTData`
...
No AWS account, no certificates, no Presidio-hosted infrastructure. See

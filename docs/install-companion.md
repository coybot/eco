...
S3 bucket or Presidio-hosted artifact needed.
...
| `presidio-network-manager` | Orin | WiFi state machine (infra WiFi ↔ AP-mode fallback) — see [`provisioning.md`](provisioning.md) |
| `presidio-local-control` | Orin | Local HTTPS API for the phone-hosted offline path — see [`offline-first.md`](offline-first.md) |
